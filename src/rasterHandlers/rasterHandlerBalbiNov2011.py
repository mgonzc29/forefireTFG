...
class RasterHandlerBalbiNov2011(RasterHandler):
    ...
    def preprocess_raster_netCDF(self, meteo_data, center_coords, radius):
        ...
        open_zone_tif_fuel=self.get_zone_over_coord(self.complete_fuel_tif,center_coords[0], center_coords[1], 'EPSG:4326', radius)
        open_zone_tif_altitude=self.get_zone_over_coord(self.altitude_tif,center_coords[0], center_coords[1], 'EPSG:4326', radius) 
        
        fuel_layer = open_zone_tif_fuel[0][0]
        #temporal:
        # a=fuel_layer[0][0]
        # fuel_layer = fuel_layer*0+a
        ...
        temperature_layer = temperature_layer + 273.15 #Convertir a Kelvin
